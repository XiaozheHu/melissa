...
options.embedding.mustlink_penalty = 10; 

% the weight of the edges connecting dummy nodes to dummy nodes
options.embedding.cannotlink_penalty = 0.0; 
...
% smash results
acc_sm = zeros(length(folds), n_dim_test);
f1_sm  = zeros(length(folds), n_dim_test);
auc_sm = zeros(length(folds), n_dim_test);
...
for i = 1:length(folds)
    ...
    fprintf('Compute Smash embedding \n');
    tic;
    %[x_smash, d_smash] = svd_embed_nolog(walks_argumented, options.embedding.ndim);
    [x_smash, d_smash] = svd_embed(walks_argumented, options.embedding.ndim);
    toc;
    ...
    for j = 1:n_dim_test
        ...
        % Perform Flame 
        %-------------------------------
        fprintf('[Perfoming Smash]\n');
        [dist_mat,knn] = compute_knn_labelled(x_smash(1:embed_dim,1:ngene), k, train_filt);
